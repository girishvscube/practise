...
import { fetchPoStatusDropdown } from '../../../../features/dropdowns/dropdownSlice'
...
const POStatus: React.FC<Props> = ({ PoId, PoById }) => {
    const { PoStatusDropdown } = useSelector((state: any) => state.dropdown)
    const { isPOstatusChanged } = useSelector((state: any) => state.purchaseOrder)
    ...
    const UpdatePoStatus = async () => {
        ...
        dispatch(ChangePoStatus(PoId, { status: params.status }))
        return true
    }

    useEffect(() => {
        if (isPOstatusChanged) {
    ...
    return (
        <div className='divstyles bg-lightbg p-4 border border-border rounded-lg'>
            <p className='subheading'>PO Status</p>
            <div className='bg-darkbg  flex flex-col gap-y-6 rounded-lg p-4 font-nunitoRegular'>
                <div className='flex pb-3 border-b  border-border items-center justify-between '>
                    <p className='text-textgray text-xs'>Current PO Status:</p>
                    <Status>{PoById?.status}</Status>
                </div>

                <SelectInput
                    // width="100%"
                    options={PoStatusDropdown}
                    handleChange={handlePoStatus}
                    value={params?.status}
                ...
                <div className='  flex justify-between '>
                    <div />
                    <CustomButton
                        disabled={disablebtn || CountItems(params) === 0}
