...
const CustomizedDialogs = ({ handleDocument, open, handleClickOpen, handleClose, handleChange, setImage, setPreview, title, type, name, additionalDoc, formErrors }: PopUpProps) => (
  <div>
    {
      type === 'success' ? null : (
        <CustomButton
    ...
    <div>
      <BootstrapDialog  sx={{
        ...
        open={open}
        PaperProps={{ sx: { width: '100%', m: 1, backgroundColor: '#404050', alignItems: 'center', borderColor: 'arsenic' } }}
      >
        ...
        <DialogContent>
          {
            type === 'success' ? (
              <div className="w-[325px] lg:w-[565px] bg-darkGray mt-6 p-2 lg:p-10 rounded-lg flex flex-col gap-10 items-center">
                <div className="flex flex-col gap-2 text-textgray text-[16px] font-nunitoRegular justify-center items-center">
              ...
              <div className="w-[325px]  lg:w-[565px] mt-6 bg-[#151929] p-2 lg:p-10 rounded-lg flex flex-col gap-10">
                ...
                <div className="w-full">
                  ...
                  />
                  {formErrors?.file && (
                    <p className='ml-4 text-errortext text-xs'>{`*${formErrors?.file}`}</p>
                  )}
                </div>
              </div>
            )
