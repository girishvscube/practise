...
import { fetchpayinList } from '../../../../features/accounts/payInSlice'
...
const PaymentsInListing = () => {
  const { accountsDropdown } = useSelector((state: any) => state.dropdown)
  const { payinList, Loading, metadata } = useSelector((state: any) => state.payin)
  const dispatch = useDispatch()
  const navigate = useNavigate()
  const [filtersCount, setFiltersCount] = useState(0)
  const { totalPayIn } = metadata
  ...
  const handleCreate = () => {
    navigate('/accounts/create/payments-in')
  }
  ...
  return (
    <div className='users-section'>
      <div className='mt-6 flex justify-between'>
        <div>
          <p className='text-xl font-extrabold text-white font-nunitoRegular'>List of Payment-in</p>
          <div className='w-full border-b border-border' />
          <p className='text-xs font-nunitoRegular mt-1 font-normal'>
            Total Payment in: {totalPayIn}
          </p>
        </div>
        ...
        <div className=' gap-6 hidden lg:flex'>
          ...
          <CustomButton
            ...
            borderRadius='8px'
          >
            <p className='font-bold font-nunitoRegular text-sm '>Record Pay-in</p>
          </CustomButton>
        </div>
      </div>
      ...
      {payinList?.length > 0 ? (
        <p className='mt-[16px] text-textgray text-xs font-nunitoRegular font-normal'>
          Showing <span className='text-white'> {payinList.length + (currentPage - 1) * 10}</span>{' '}
          out of <span className='text-white'>{totalPayIn}</span> results
        </p>
      ) : null}
      ...
      {Loading ? (
        ...
        <div>
          <div className='w-full  bg-[#404050] rounded-lg mt-[16px]'>
            <BasicTable cols={cols} data={payinList} />
          </div>

          <Pagination
            className='pagination-bar'
            currentPage={currentPage}
            totalCount={metadata.totalPayIn}
            pageSize={10}
...
export default PaymentsInListing
