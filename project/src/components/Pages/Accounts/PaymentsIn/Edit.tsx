...
const EditPaymentsIn = () => {
    ...
    const handleSubmit = () => {
        ...
        setLoading(true)
        axiosInstance.put(`/admin/pay-in/${decryptData(id)}`, params).then(() => {
    ...
    const fetchInvoiceById = (id: any) => {
        setLoading(true)
        axiosInstance.get(`/admin/pay-in/${decryptData(id)}`).then((res) => {
            setParams({
                ...params,
                customer_id: res.data.data?.payin?.customer_id,
                bank_account_id: res.data.data?.payin?.bank_account_id,
                pay_in_date: moment(res.data.data?.payin?.pay_in_date).format('YYYY-MM-DD'),
                amount: res.data.data?.payin?.amount,
                notes: res.data.data?.payin?.notes,
            })
            let list = res.data.data?.payin_invoices.map(x => x.order_payment.balance)

            if (list.length > 0) {
                list.map((x) => {
                    x['id'] = x?.listOfInvoice.id,
                        x['amount'] = x?.listOfInvoice.balance,
                        x['total_amount'] = x?.order_payment?.grand_total,
                        x['balance'] = x?.order_payment?.balance
                })
                setInvoice(list)
            }
            setLoading(false)
